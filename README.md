```
 __  __            _                  _____ _____ _      _      ______
|  \/  |          (_)                / ____|_   _| |    | |    |  ____|
| \  / | __ ___  ___ _ __ ___   ___ | |  __  | | | |    | |    | |__
| |\/| |/ _` \ \/ / | '_ ` _ \ / _ \| | |_ | | | | |    | |    |  __|
| |  | | (_| |>  <| | | | | | |  __/| |__| |_| |_| |____| |____| |____
|_|  |_|\__,_/_/\_\_|_| |_| |_|\___| \_____|_____|______|______|______|
```

**`$ whoami`** → Analyste Cybersécurité | Passionné par la tech & les jeux vidéo 🎮🔐

---

### 🛡️ À propos

```bash
root@maximegille:~$ cat /usr/share/info/about.txt
```

Analyste cybersécurité avec **3 ans d'expérience** en alternance chez **Icade**.  
Spécialisé dans l'analyse de menaces, la réponse aux incidents et l'implémentation de solutions de sécurité.

🎓 **Mastère Cybersécurité (Bac+5)** - ESIEE-IT  
📍 Val-d'Oise, France

---

### 🔧 Compétences Techniques

```bash
root@maximegille:~$ ls -la /skills/
```

| Domaine | Technologies |
|---------|-------------|
| **SIEM / SOC** | Exabeam, Crowdstrike, Tehtris, Cyberwatch |
| **Analyse Réseau** | Wireshark, TCPdump |
| **Pentest** | Metasploit, Burp Suite, Nmap |
| **Scripting** | Bash, PowerShell, C, HTML |
| **Infra** | Proxmox, Nextcloud, VPN, Serveurs Web |

---

### 📂 Projets

```bash
root@maximegille:~$ find /projects -type f -name "*.sec"
```

| Projet | Description | Stack |
|--------|-------------|-------|
| **MiniTeams-ESIEE-IT** | Clone de Microsoft Teams (version initiale) | C, Shell |
| **My_Teams-ESIEE-IT** | Clone de Microsoft Teams (version évoluée) | C |
| **MoneyKiiz** | Gestion de projet V2 - App mobile avec chatbot & analyse sécurité | iOS, Android |
| **Homelab** | Infra personnelle : Proxmox, Nextcloud, VPN, Gaming Server | Proxmox, SSL |
| **KeePass Migration** | Migration de gestionnaire de secrets en entreprise | Sécurité |

---

### 🏆 Expériences

```bash
root@maximegille:~$ git log /jobs --oneline
```

- **2023-2026** → Analyste Cybersécurité @ **Icade**
- **2023-2026** → Membre @ **Raiki** (Association E-sport)
- **2021-2023** → Technicien Support @ **Icade**
- **2024-2026** → Responsable Réseau @ **Global Game Jam Cergy**

---

### 📜 Certifications & Diplômes

```bash
root@maximegille:~$ cat /etc/certs/list.txt
```

- ✅ Mastère E3IN Cybersécurité (RNCP39425) - ESIEE-IT
- ✅ Licence Informatique Générale - CNAM
- ✅ Bachelor E3IN - ESIEE-IT
- ✅ BTS SIO SISR - ESIEE-IT

---


